...
const size = WIDTH / 30
const r = size / 10
const COLOR_STEPS = WIDTH
const SUNSET_STEPS = WIDTH
...
const colors = ColorSteps.getColorSteps('#2a6f97', '#a9d6e5', COLOR_STEPS)
const reversed = ColorSteps.getColorSteps('#a9d6e5', '#2a6f97', COLOR_STEPS)
const sunset = ColorSteps.getColorSteps("#e9c46a", "#e76f51", SUNSET_STEPS)
const sunsetReversed = ColorSteps.getColorSteps("#e76f51", "#e9c46a", SUNSET_STEPS)

function drawLandscape() {
    const noiseLevel = 100

    clear()
    background(sunset[sunset.length - 1])

    for (let i = 0; i < WIDTH; i += r) {
    for (let j = 0; j < HEIGHT; j += r) {
        const s = noise(j) * random(size / 3, size / 2)
        const c = parseInt(i % SUNSET_STEPS)
        push()
        strokeWeight(3)
        stroke(sunsetReversed[c])
        rotate(radians(noise(i) * random(30, -30)))
        fill(sunset[c])
        rect(i, j, s, s)
        pop()
    }
    ...
    for (let i = WIDTH; i > 0; i -= r) {
    for (let j = HEIGHT / 3; j > (HEIGHT / 1); j -= r / 2) {
        const s = noise(j) * random(size / 3, size / 2)
        const c = parseInt(i % COLOR_STEPS / 7)
        push()
        //noStroke()
        strokeWeight(0.5)
        stroke(reversed[c])
        fill(colors[c])
        rotate(radians(noise(i) * random(-50, -80)))
        rect(i, j + noise(j) * random(-noiseLevel, noiseLevel), s, s)
        pop()
    }
    }

    for (let i = 0; i < WIDTH * 7; i += r) {
    for (let j = HEIGHT / 2 + HEIGHT / 3; j < HEIGHT; j += r) {
        const s = noise(j) * random(size / 2, size)
        const c = parseInt(i % COLOR_STEPS / 5)
        push()
        //noStroke()
        strokeWeight(0.5)
        stroke(colors[c])
        fill(reversed[c])
        rotate(radians(noise(i) * random(20, -60)))
        rect(i, j + noise(j) * random(-noiseLevel, noiseLevel), s, s)
        pop()
    }
    }
    ...
    for (let j = HEIGHT / 2 + HEIGHT / 3; j < HEIGHT; j += r) {
        ...
        stroke(reversed[c])
        fill(colors[c])
        rotate(radians(noise(i) * random(50, -60)))
        rect(i, j + noise(j) * random(-noiseLevel, noiseLevel), s, s)
        pop()
    }
    }
